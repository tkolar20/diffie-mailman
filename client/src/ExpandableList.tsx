...
const ExpandableList: React.FC<ExpandableListProps> = ({ emails }) => {
  ...
  return (
    <div className="list-container" style={{ maxHeight: 'calc(100vh - 20px)', overflowY: 'auto' }}>
    <ul className="list-group mt-3">
      {emails.map((email) => (
        <li
          key={email.id}
          className={`list-group-item clickable${openEmailId === email.id ? ' active' : ''}`}
          onClick={() => handleToggle(email.id)}
        >
          <div className="d-flex justify-content-between align-items-center">
            <div>
                <strong>{email.subject}</strong>
                <span className="ms-2">From: {email.sender}</span>
                {email.seen ? (
            ...
            >
              <i className={`bi bi-chevron-${openEmailId === email.id ? 'up' : 'down'}`}></i>
            </Button>
          </div>
